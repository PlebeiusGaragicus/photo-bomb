...
#   7. Smoke-test the bundle: `Photo Boss.app/Contents/MacOS/Photo Boss --version`
#   8. Build dist/Photo-Boss-<version>-arm64.dmg via create-dmg (or hdiutil
...
if [[ "${ARCH}" != "arm64" ]]; then
    ...
    echo "       (run on Apple Silicon, or update target_arch in packaging/photo_boss.spec)" >&2
    exit 1
fi

PYTHON="${PYTHON:-python3}"
VENV_DIR="${REPO_ROOT}/.venv-build"
SPEC="${REPO_ROOT}/packaging/photo_boss.spec"
ENTITLEMENTS="${REPO_ROOT}/packaging/entitlements.plist"
ICON_SCRIPT="${REPO_ROOT}/packaging/make_icon.sh"
ICON_PATH="${REPO_ROOT}/src/photo_boss/assets/icons/icon.icns"
APP_PATH="${REPO_ROOT}/dist/Photo Boss.app"
...
SMOKE_BIN="${APP_PATH}/Contents/MacOS/Photo Boss"
if [[ ! -x "${SMOKE_BIN}" ]]; then
...
VERSION="$(python -c 'import re,sys; print(next(re.search(r"__version__\s*=\s*[\"\x27]([^\"\x27]+)", l).group(1) for l in open("src/photo_boss/__init__.py") if l.startswith("__version__")))')"
DMG_PATH="${REPO_ROOT}/dist/Photo-Boss-${VERSION}-arm64.dmg"
rm -f "${DMG_PATH}"

if command -v create-dmg >/dev/null 2>&1; then
    echo "[dmg] building ${DMG_PATH} via create-dmg"
    create-dmg \
        --volname "Photo Boss ${VERSION}" \
        --window-pos 200 120 \
        --window-size 540 320 \
        --icon-size 100 \
        --icon "Photo Boss.app" 140 150 \
        --hide-extension "Photo Boss.app" \
